...
EOL;
    ...
    public function exportOrders()
    {
        ...
        $skuNumbers = Products::join('order_products', 'products.id', '=', 'order_products.product_id')
            ->distinct()
            ->pluck('products.sku_number');

        $statuses = Orders::join('order_products AS op', 'orders.id', '=', 'op.order_id')
            ->distinct()
            ->pluck('orders.status');

            $categories = OrderProductRelation::join('products', 'order_products.product_id', '=', 'products.id')
            ->join('product_categories', 'products.category_id', '=', 'product_categories.id')
            ->selectRaw("DISTINCT(product_categories.id) as category_id, product_categories.title as category_title")
            ->get();

            $brands = OrderProductRelation::join('brands', 'order_products.brands_id', '=', 'brands.id')
            ->distinct()
            ->select(['brands.id', 'brands.title'])
            ->get();

        $sizes = OrderProductRelation::distinct()->pluck('size_title');

        $colors = OrderProductRelation::distinct()->pluck('color');
    ...
    function export(Request $request)
    {
        ...
        $query = OrderProductRelation::join('products', 'order_products.product_id', '=', 'products.id')
            ...
            ->select([
                ...
                'orders.id as invoiceID',
                'orders.status',
               'product_categories.title as category',
               'brands.title as brand',
                DB::raw("IFNULL(JSON_UNQUOTE(JSON_EXTRACT(order_products.logo_data, '$[0].category')), '') as logo"),
            // 'orders.brand',
            'products.purchase_price',
            'products.price as selling_price',
            'products.sale_price',
            'orders.logo_cost as logo_price',
            'orders.total_amount as total_price',
            ]);

        if (!empty($data['skuNumber'])) {
            $query->whereIn('products.sku_number', $data['skuNumber']);
        }

        if (!empty($data['colors'])) {
            $query->whereIn('order_products.color', $data['colors']);
        }

        if (!empty($data['sizes'])) {
            $query->whereIn('order_products.size_title', $data['sizes']);
        }

         if (!empty($data['product_name'])) {
             $query->where('order_products.product_title', 'LIKE', '%' . $data['product_name'] . '%');
        }
        ...
        if (!empty($data['brands'])) {
            $query->whereIn('order_products.brands_id', $data['brands']);
        }
        ...
        $callback = function () use ($listing) {
            ...
            fputcsv($file, ['SKU', 'Name', 'Color', 'Size', 'Selling Date', 'Invoice No', 'Category', 'Brand','Logo','Status',
                                    'Purchase Price', 'Selling Price', 'Sale Price', 'Logo Price', 'Total Price']);
            ...
            foreach ($listing as $row) {
                fputcsv($file, [
                    ...
                    number_format($row->logo_price ?? 0, 2),
